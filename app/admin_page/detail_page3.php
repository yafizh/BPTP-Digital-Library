    ...
    <main class="container">
        <div class="d-flex">
            <div class="flex-grow-1">
                <h2 id="category-title">Laporan Pengunjung Website</h2>
            </div>
            ...
            <table class="table table-borderless table-hover">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Buku Yang Dibaca</th>
                    </tr>
                    <tr>
        ...
        $.ajax({
            ...
            data: {
                'query': `
                    SELECT
                        website_guest_id, 
                        DATE(website_guest_table.website_guest_date_time_enter) AS date_enter,
                        TIME_FORMAT(website_guest_table.website_guest_date_time_enter, '%H:%i') AS time_enter 
                    FROM website_guest_table
                `
            },
            dataType: 'json',
            async: false,
                        ...
                        $.ajax({
                            ...
                            data: {
                                'query': `
                                    ...
                                    FROM website_book_views_table INNER JOIN book_table ON book_table.book_id=website_book_views_table.book_id WHERE 
                                    website_guest_id=${value.website_guest_id} GROUP BY book_table.book_id
                                `
                            },
                            dataType: 'json',
                            async: false,
                                ...
                                if (response2.isSuccess) {
                                    let z = `
                                        <tr>
                                            ...
                                            <td><label class="ms-1">${value.time_enter}</label></td>
                                            <td><label class="ms-1">${dateToIndonesiaDateFormat(value.date_enter)}</label></td>
                                            <td><label class="ms-1">`
                                    if (response2.data.length) {
                                        $.each(response2.data, function(index2, value2) {
                                            if(value2 == response2.data[response2.data.length-1]){
                                                z += `${value2.book_title}`
                                            } else {
                                                z += `${value2.book_title}, `
                                            }
